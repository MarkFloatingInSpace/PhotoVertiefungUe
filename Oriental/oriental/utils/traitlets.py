...
"""
import unittest
# import all, so application code does not need to import traitlets in addition to this file
...
class HasStrictTraits(HasTraits):
    ...
    def __setattr__(self, name, value):
        if hasattr(self,'_init') and not hasattr(self,name):
            raise TraitError( "Must not create additional attribute {} on instance of HasStrictTraits after initialization".format(name) )
...
class NDArray(Instance):
    ...
    def __init__(self, **metadata):
        ...
        super().__init__(
            ...
            #args=( (abs(x) for x in metadata.get('shape', ())), ),
            #args=( tuple(abs(x) for x in metadata.get('shape', ())), ),
    ...
    def validate(self, obj, value):
        ...
        val = super().validate(obj, value)

        # maybe check dtype castable
        # TODO exact dtype match
        dt = self.get_metadata('dtype')
        if dt:
            if not np.can_cast(np.dtype(dt), value.dtype):
                msg = 'Expected type castable with %r, received %r'
                msg %= (np.dtype(dt), value.dtype)
    ...
    def __set__(self, obj, value):
        ...
        if self.eq == 'id':
            ...
            try:
                same = (old==new).all() if self.eq=='all' else np.allclose(old, new)
            except ValueError:
                same = False
...
class TestValidation(unittest.TestCase):
    ...
    def test_shape_bcast(self):
        tb = self.TestBed()
        tb.C = np.zeros((2, 3, 4))
        tb.C = np.zeros((5, 4, 3, 1))
        tb.C = np.zeros((1, 3, 1, 2))
        tb.C = np.zeros((1, 2, 3, 4))
        with self.assertRaises(TraitError):
            tb.C = np.zeros((1, 3, 2, 2))
